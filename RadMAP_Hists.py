...
# set these before possibly modifying depending on avail weather data
start_event = 0
n_events = len(event)
...
if weather_avail:
  ...
  try:
    x = first_bin_time  # no weather even close if first bin time wasnt assigned (end of month)
  except NameError:
    print '*****************\n','Weather not available for this run\n','*****************'
    weather_avail = False

if weather_avail:
  if first_bin_time > 300. and last_bin_time < 300.:  # set first_bin_time to 60 and find new ev_index to strart main loop with
    first_bin_time = 60.
    print "Starting at first event with weather data..."
    for i in range(0,n_events):
      ev_time = float(event[i].split(',')[2])
      if ev_time > first_bin_utc:
        print "No weather data before event",i
        start_event = i
        break
...
last_ev_time = float(event[start_event].split(',')[2])
prev_alt = float(event[start_event].split(',')[5])
...
def findKp(epochtime,idx):
  ...
  try: 
    ...
    print 'UTC time: ',unix2utc(epochtime)
    print '*****************************'
    Kpval = -1
    geomag_data = False
  if geomag_data:
...
def Check_DTOAK(ev_lat,ev_lon):
  oak_poly = Polygon(((37.80035833333333,-122.27822222222223),
    (37.81259444444444,-122.27016388888889),
    (37.81105833333333,-122.26224166666667),
    (37.80363055555555,-122.26038333333334),
    (37.79920277777778,-122.26260555555555),
    (37.801738888888885,-122.26973611111111),
    (37.79804722222222,-122.27221666666667)))
...
if weather_avail:
  # first event weather
  ev_weather = getWeather(beg_time,0)
...
FRN_geomag = findMagRange(beg_time)
...
if lidar_avail:
  lidar_frac = getLidar(beg_time,0)
...
prev_daytime = int(unix2utc(beg_time)[11:13])+float(int(unix2utc(beg_time)[14:16])/60.)-8
if prev_daytime < 0:
...
prev_min = int(unix2utc(beg_time)[-5:-3])
