...
if not new_gps_avail[0]:
  ...
  #  if(Event.bayes_prob_n() > min_n_prob and Event.rawamplitude() < 4000 and Event.amplitude() > 6.5 and Event.channel() != chprob1 and Event.channel() != chprob2):
    if(Event.llr_n() >= min_n_llr and Event.rawamplitude() < 4010 and Event.amplitude() > 6.5 and Event.channel() != chprob1 and Event.channel() != chprob2):
      ...
      #else:
      neutron_events.append([Event.channel(),'%.9f' % Event.epoch_time(),Event.gps_lat(),Event.gps_long(),Event.gps_alt(),ent])
  ...
  for i in range(0, len(sorted_events)):
    ev_alt = sorted_events[i][4]+altcorr
    ...
    if(Event.llr_n() >= min_n_llr and Event.rawamplitude() < 4010 and Event.amplitude() > 6.5 and Event.channel() != chprob1 and Event.channel() != chprob2):
      ...
      for i in range(idx,len(gpstime)):
          ...
          if (ev_time < gpstime[0]):
              neutron_events.append([Event.channel(),'%.9f' % Event.epoch_time(),lat[0],lon[0],'%.3f' % elev[0],ent]);
              break
          if (gpstime[i] >= ev_time):
              ...
              neutron_events.append([Event.channel(),'%.9f' % Event.epoch_time(),ev_lat,ev_lon,'%.3f' % ev_alt,ent]);
              # find a way to do this... idx = i # assign next index so we dont need to sort through all previous data on the next event
              break
          if(i == len(gpstime)-1):
              neutron_events.append([Event.channel(),'%.9f' % Event.epoch_time(),lat[i],lon[i],'%.3f' % elev[i],ent]);
